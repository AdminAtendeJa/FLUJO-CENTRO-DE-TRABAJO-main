...
export default function TramitesSettings() {
  ...
  const [editForm, setEditForm] = useState({ nombre: '', codigo: '', activo: true });
  ...
  const handleEditClick = (t) => {
    setEditingId(t.id);
    setEditForm({ nombre: t.nombre, codigo: t.codigo || '', activo: t.activo });
  };
  ...
  const handleAddSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await createCatalogoTramite({ nombre: newForm.nombre, codigo: newForm.codigo, activo: true });
      toast.success('Trámite agregado');
      setIsAdding(false);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h2 style={{ margin: 0, fontSize: '1.25rem', color: 'var(--color-text-primary)' }}>Catálogo de Trámites</h2>
      ...
      <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
                ...
                {editingId === t.id ? (
                  ...
                  <>
                    ...
                    <td style={{ padding: '0.75rem 1rem', textAlign: 'right' }}>
                      <button onClick={() => handleEditClick(t)} className="btn btn-ghost" style={{ padding: '0.25rem' }}><Edit2 size={16} /></button>
                    </td>
                  </>
                )}
              </tr>
            ...
            {tramites.length === 0 && (
              <tr><td colSpan="4" style={{ textAlign: 'center', padding: '1rem', color: 'var(--color-text-secondary)' }}>No hay trámites definidos</td></tr>
            )}
          </tbody>
        </table>
      </div>
